...
\usepackage{amsthm}
\renewcommand{\proofname}{\textbf{証明}} % proof 環境の見出し（既定は "Proof"）
...
\newtheoremstyle{definitionbreak}%
  ...
  {\newline}%
  {\thmname{#1}\thmnumber{ #2}\thmnote{: #3}}%
% --- definitioninline: definitionbreak の run-in 版（見出し後に改行しない） ---
%     見出しは「名前 番号」のみ（タイトル・句点なし），見出しに続けて本文を組む．
%     練習問題（exc）用．「練習問題 N.M 問題文」の書式になる
\newtheoremstyle{definitioninline}%
  ...
  {}%          見出し末（句点なし）
  { }%         見出し後はスペース（改行しない）
  {\thmname{#1}\thmnumber{ #2}}% タイトル（note）は出さない
...
% --- solution: 解答用．amsthm 標準 definition と同じ run-in だが見出し上の
%     スペースを半分にして直前の練習問題（ボックス）と詰める ---
%     jp は exc と同様に見出し末の句点を付けない（「解」）．en では `.` を残す
\newtheoremstyle{solution}%
  {0.5\topsep}% 見出し上スペース（definition の \topsep より小さく）
  {\topsep}%    本文下スペース
  {\normalfont}% 本文は通常フォント
  {}%
  {\bfseries}%  見出し太字
  {}%           見出し末（句点なし．「解」）
  {.5em}%       見出し後はスペース（run-in）
  {}%           見出し書式は既定（名前 番号）
...
\AddToHook{env/Verbatim/before}{\callout@fixthmheadbreak}
\makeatother
% 定理系環境の定義と登録は Callout block 内の \NewCalloutThmEnv /
% \NewCalloutLawEnv で行う（定義 \newtheorem と登録 \RegisterCalloutEnv を
% 1コマンドにまとめたもの）
{% endraw -%}
%%%% Callout block %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
{% raw -%}
...
% --- ボックス直後の文を新しい段落にしない ---
%     リスト環境と同様，\end の直後に空行を挟まずに続けた文は同じ段落の続き
%     （字下げなし）として組む．カーネルの @endpe 機構（\@doendpe）をボックス
%     直後に適用する．callout / objective 等のラッパー環境は内部で calloutbox
%     を閉じるため，カーネルの \aftergroup 連鎖で外側の \end まで自動的に
%     伝播する（空行を挟んだ場合は従来どおり新しい段落になる）
\makeatletter
...
% 名前に * を含むか（含む→#2，含まない→#3 を実行）
\def\callout@ifstarred#1{\callout@ifstarred@#1*\@nil}
\def\callout@ifstarred@#1*#2\@nil{\ifblank{#2}{\@secondoftwo}{\@firstoftwo}}
% \RegisterCalloutEnv{環境名}{枠色}[タイトル渡し方式 optional / mandatory]
%   環境名を callout 用に登録する．番号なし版「環境名*」も同じ設定で自動登録
%   される（環境名が * 付きなら何もしない）．明示登録は記録され，自動登録より
%   常に優先される（記述の順序によらない）．
%   登録はあくまで対応表への追加であり，環境自体は別途定義が必要
%   （定義込みで行うなら後述の \NewCalloutThmEnv / \NewCalloutLawEnv を使う）
...
% --- 定理系環境の新設ヘルパー: 定義（\newtheorem）+ 登録を1コマンドで ---
% \NewCalloutThmEnv{環境名}[番号共有元]{表示名}{枠色}[番号のリセット単位]
%   番号付き環境と番号なし環境（環境名*）を作り，両方を登録する．
%   オプションは \newtheorem と同じ規則（排他）:
%     [番号共有元]      … 指定した環境と通し番号（例: [thm]）
%     [リセット単位]    … 独立番号で，そのカウンタごとにリセット
%                         （例: [section] / [subsection]）
%     両方省略          … 独立番号・リセットなし（文書全体で 1, 2, ...）
%   定理スタイルは \newtheorem と同様，呼び出し時点の \theoremstyle に従う
\NewDocumentCommand{\NewCalloutThmEnv}{ m o m m o }{%
  \IfValueTF{#2}
    {\IfValueTF{#5}
       {\GenericError{}{\string\NewCalloutThmEnv: 番号共有元とリセット単位は
          同時に指定できません}{}{}}
       {\newtheorem{#1}[#2]{#3}}}
    {\IfValueTF{#5}
       {\newtheorem{#1}{#3}[#5]}
       {\newtheorem{#1}{#3}}}%
  \newtheorem*{#1*}{#3}%
  \RegisterCalloutEnv{#1}{#4}% 環境名* も自動登録される
}
% \NewCalloutLawEnv{環境名}{枠色}
%   law 型（タイトル必須・番号なし・lawbreak スタイルでタイトルのみを見出しに
%   表示）の環境を作って登録する．* 版は作らない
\NewDocumentCommand{\NewCalloutLawEnv}{ m m }{%
  \edef\callout@savedthmstyle{\the\thm@style}%
  \theoremstyle{lawbreak}%
  \newtheorem*{#1@inner}{#1}% 内部用．名前は表示されない（\thmname を使わないため）
  \expandafter\thm@style\expandafter{\callout@savedthmstyle}%
  \NewDocumentEnvironment{#1}{ m }{%
    \begin{#1@inner}[##1]%
  }{%
    \end{#1@inner}%
  }%
  \callout@register{#1}{#2}{mandatory}% * 版を登録しないので低レベル登録
  \@namedef{callout@explicit@#1}{}%
}
\makeatother
% 定理系環境の定義と登録（番号なし版 thm* 等も同時に作成・登録される）
\theoremstyle{definitionbreak}
%\theoremstyle{definition} % 改行なしスタイル
\NewCalloutThmEnv{thm}{定理}{frost1}[section]
\NewCalloutThmEnv{lem}[thm]{補題}{frost1}
\NewCalloutThmEnv{prop}[thm]{命題}{night4}
\NewCalloutThmEnv{cor}[thm]{系}{night4}
\NewCalloutThmEnv{conj}[thm]{予想}{night4}
\NewCalloutThmEnv{asm}[thm]{仮定}{nordviolet}
\NewCalloutThmEnv{dfn}[thm]{定義}{nordviolet}
\NewCalloutThmEnv{rem}[thm]{注}{night4}
% 練習問題の見出し名（\renewcommand で「問」「問題」等へ変更可）
\newcommand{\excname}{練習問題}
% 練習問題は definitioninline（見出し「練習問題 N.M」＋スペースで本文 run-in）
\theoremstyle{definitioninline}
\NewCalloutThmEnv{exc}{\excname}{nordgreen}[section]
% --- excb: exc の改行版（thm と同じ definitionbreak スタイル）．見出し
%     「練習問題 N.M」を太字で出し，オプションタイトルがあれば「: タイトル」を
%     続け，改行してから本文を組む．タイトルは任意（必須ではない）．
%     番号は exc とカウンタを共有（[exc]）するので混在しても通し番号になる ---
\theoremstyle{definitionbreak}
\NewCalloutThmEnv{excb}[exc]{\excname}{nordgreen}
% --- 解答環境（proof 型・番号なし・run-in・見出し太字「解．」） ---
%     練習問題環境の内外いずれでも使用可．proof と異なり QED 記号は付けない
%     見出しはオプション引数で個別指定でき（例 \begin{sol}[略解] → 「略解．」），
%     省略時は \solname（既定「解」．\renewcommand{\solname}{…} で全体の既定を変更）
\newcommand{\solname}{解}
\newcommand{\solheading}{\solname} % 現在の見出し（sol 環境が局所的に上書き）
...
\theoremstyle{definitionbreak} % law（タイトル後に改行）の手前で既定スタイルに戻す
